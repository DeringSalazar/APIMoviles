    ...
    async getAllHerramientas() {
        try {
            const { rows } = await pool.query(`
                ...
                WHERE dc.category_slug = '07'
            `);
            return rows || [];
        } catch (error) {
            console.error('Error en getAllHerramientas:', error);
            throw error;
        }
    }

    async getHerramienta(productId) {
        try {
            ...
            console.error('Error en getHerramienta:', error);
            throw error;
        }
    }

    async getHerramientaPdfPage(productId) {
        try {
            const { rows } = await pool.query(`
                SELECT 
                    prod ->> 'id' AS product_id,
            ...
            console.error('Error en getHerramientaPdfPage:', error);
            throw error;
        }
    }
}

module.exports = new Herramientas();