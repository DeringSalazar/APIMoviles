  ...
  async getAllQuimicosCards() {
    try {
      const { rows } = await pool.query(`
        ...
        WHERE dc.category_slug = '02'
      `);
      return rows || [];
    } catch (error) {
      console.error('Error en getAllQuimicosCards:', error);
      throw error;
    }
  }

  async getQuimicosDetail(productId) {
    try {
      const { rows } = await pool.query(`
        SELECT 
          dc.content::jsonb ->> 'name' AS category_name,
      ...
      console.error('Error en getQuimicosDetail:', error);
      throw error;
    }
  }

  async getQuimicosPdfPage(productId) {
    try {
      const { rows } = await pool.query(`
        SELECT 
          prod ->> 'id' AS product_id,
      ...
      console.error('Error en getQuimicosPdfPage:', error);
      throw error;
    }
  }
}

module.exports = new QuimicosModel();