...
class MaquinasModel {
  async getAllMaquinasCards() {
    try {
      ...
      console.error('Error en getAllMaquinasCards:', error);
      throw error;
    }
  }

  async getMaquinasDetail(productId) {
    try {
      ...
      console.error('Error en getMaquinasDetail:', error);
      throw error;
    }
  }

  async getMaquinasPdfPage(productId) {
    try {
      const { rows } = await pool.query(`
        SELECT 
          prod ->> 'id' AS product_id,
      ...
      console.error('Error en getMaquinasPdfPage:', error);
      throw error;
    }
  }
}
